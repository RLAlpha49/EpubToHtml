...
def convert_batch(
    inputs: tuple[Path, ...], template: ConversionOptions, output_root: Path, workers: int = 1
) -> BatchResult:
    ...
    def one(path: Path) -> BatchItemResult:
        try:
    ...
    if workers == 1:
        return BatchResult(tuple(one(path) for path in paths))
    with ThreadPoolExecutor(max_workers=workers) as executor:
        return BatchResult(tuple(executor.map(one, paths)))
