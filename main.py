...
def epub_to_html(epub_path, html_path):
    ...
    for item in book.get_items():
        if item.get_type() == ebooklib.ITEM_IMAGE:
            ...
            img_tags[
                item.get_name()] = f'<img src="data:image/{item.media_type.split("/")[-1]};base64,{img_data}" style="max-width: 100%; height: auto;">'

    for item in book.get_items():
        if item.get_type() == ebooklib.ITEM_DOCUMENT:
            content = item.get_content().decode('utf-8')
            for img_name, img_tag in img_tags.items():
                content = re.sub(r'src=["\'].*?' + re.escape(img_name) + '.*?["\'].*?>', img_tag, content)
...
parser.add_argument('--html_path', type=str, default=os.getcwd() + '/output.html',
